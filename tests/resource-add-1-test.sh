...
it_creates_a_resource() {
    rerun rundeck-admin:resource-add $CREDS --project test-$$ --data "-name gilligan -description 'silly sailor'"
}

it_updates_a_resource() {
    CREDS="--user admin --password admin --url http://localhost:4440"
    rerun rundeck-admin:resource-add $CREDS --project test-$$ --data "-name gilligan -description 'silly sailor' -osFamily comedian -blar blar"
}
